...
def checksum(spreadsheet):
    ...
    for row in spreadsheet:
        ...
        index2 = 1
        divisor = 0
        while index1 < len(row)-1:
            #Iterate over subsequent numbers in the list
            index3 = index2
            j = 1
            #Condition to end while loop comparing numbers
            while j < (len(row)-index1):
                if row[index1] % row[index3] == 0 or row[index3] % row[index1] == 0:
                    divisor1 = row[index1] / row[index3]
                    divisor2 = row[index3] / row[index1]
                    if divisor1 >= 1:
                        divisor = divisor1
